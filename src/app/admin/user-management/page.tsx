...
import { Users, Crown, Shield, User, Search, AlertTriangle } from 'lucide-react';
...
export default function UserManagementPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: 'customer' | 'staff' | 'manager' | 'admin') => {
    if (!userProfile) return;

    // Prevent self-demotion if last admin
    if (userId === userProfile.uid && userProfile.role === 'admin' && newRole !== 'admin') {
      const adminCount = users.filter(u => u.role === 'admin').length;
      if (adminCount <= 1) {
        toast({
          variant: 'destructive',
          title: 'Cannot Demote Last Admin',
          description: 'You must have at least one admin. Promote another user to admin first.',
        });
        return;
      }
    }
            ...
            <Card className="mb-6 border-amber-200 bg-amber-50">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-amber-800 font-medium">Low Admin Count Warning</p>
                    <p className="text-amber-700 text-sm">You have only {adminCount} admin(s). Consider promoting another user to admin to avoid lockout risk.</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b">
                        ...
                        <th className="text-left py-3 px-2 font-medium text-gray-900">Current Role</th>
                        <th className="text-left py-3 px-2 font-medium text-gray-900">Store</th>
                        ...
                        <tr key={user.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="py-4 px-2">
                            <div className="flex justify-center gap-2">
                              {(['customer', 'staff', 'manager', 'admin'] as const).map((role) => (
                                <Button
                                  key={role}
                                  size="sm"
                                ...
                                >
                                  {updating === user.id ? '...' : role.charAt(0).toUpperCase() + role.slice(1)}
                                </Button>
                              ))}
                            </div>
                          </td>
